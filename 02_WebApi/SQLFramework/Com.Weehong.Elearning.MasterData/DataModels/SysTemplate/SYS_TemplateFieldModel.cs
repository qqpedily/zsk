...
namespace Com.Weehong.Elearning.MasterData.DataModels.SysTemplate
{
    ...
    public class SYS_TemplateFieldModel
    {

        public SYS_TemplateFieldModel Clone()
        {
            return (SYS_TemplateFieldModel)this.MemberwiseClone();
        }
        /// <summary>
        /// 模板字段ID
        /// </summary>
        ...
        /// </summary>
        [NotMapped]
        public List<SYS_MetaData> sYS_MetaData
        {
            get
            {
                SYS_MetaDataAdapter sYS_MetaDataAdapter = new SYS_MetaDataAdapter();
                return sYS_MetaDataAdapter.GetAll().Where(w => w.MetaDataID == this.UUID).ToList();
            }
        }
        /// <summary>
        /// 模板类型 模板
        /// </summary>
        [NotMapped]
        public List<SYS_Template> sYS_Template
        {
            get
            {
                SYS_TemplateAdapter sYS_TemplateAdapter = new SYS_TemplateAdapter();
                return sYS_TemplateAdapter.GetAll().Where(w => w.TemplateID == this.UUID).ToList();
            }
        }
